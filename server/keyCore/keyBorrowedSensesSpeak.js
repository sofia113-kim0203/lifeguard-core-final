...
export function buildCurrentInsuranceProductShowcaseAddendum({ question = "" } = {}) {
  ...
  return [
    ...
    "질환 통계·발병률·유병률·빈도·비율·퍼센트·시장 수치·위험률 등 정량 공개 수치는 이번 턴 web_search 또는 권위 있는 공개 출처에서 그 수치가 실제로 확인된 경우에만 사실처럼 말한다. 확인되지 않은 숫자를 기억·추정·합성·보간으로 채우지 않는다. 근거 없는 정량 프레이밍을 만들지 않는다. 정확한 수치가 확인되지 않으면 숫자를 생략하고 질적으로만 설명한다. 검색으로 확인된 수치는 그대로 사용해도 된다.",
