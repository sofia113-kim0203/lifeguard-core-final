...
export function buildClaudeFullSystemPrompt({ mode = "emit", documentDirect = false } = {}) {
  ...
  lines.push(
    ...
    "visual_blocks: optional. Use a table/card only when comparison is clearer that way. Never default every insurance answer to a table. Allowed types: premium_summary_table, policy_count_summary, coverage_gap_table, next_steps_card, coverage_status_card. Use ONLY verified known/unknown rows. Never invent coverages or fake comparison tables. Do not force blocks on simple confirmations or unrelated chit-chat.",
