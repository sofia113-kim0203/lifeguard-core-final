/**
 * ONE KEY Core S02-2 — Upload 3-beat matrix (document + analysis_complete Core · return legacy).
 *
 * Usage:
 *   node scripts/one-key-core-s02-2-upload-3beat-preview-probe.mjs [preview-url]
 */
...
const OUT = join(ROOT, "fixtures/key-judgment-validation-v1/one-key-core-s02-2-upload-3beat-preview-evidence.json");
...
async function resolveLatestDocumentId({ previewBase, token, bypass }) {
  const unified = await postJson({
    ...
    body: { last_event: "s02_2_upload_3beat_probe" },
  });
...
async function main() {
  ...
  const report = {
    schema_version: "one-key-core-s02-2-upload-3beat-preview-v1",
  ...
  if (documentId) {
    const docProbe = await postJson({
      ...
      body: { document_id: documentId, upload_source: "s02_2_probe" },
    });
  ...
  if (jobId) {
    ...
    const coreSteps =
      acProbe.payload?.intake_trace?.one_key_core_trace?.steps?.map((r) => r.step) ?? [];
    report.beats.push({
      ...
      customer_initiative_sentence_preview: String(
        acProbe.payload?.customer_initiative_sentence ?? "",
      ).slice(0, 240),
    ...
    report.beats.push({
      beat: "analysis_complete",
      probe_ok: false,
      core_route: "legacy_expected_without_flag",
    });
  }
  ...
  report.beats.push({
    ...
    core_route: isCoreSource(returnProbe.payload?.response_source, "return_judgment") ? "one_key_core" : "legacy",
