/**
 * ONE KEY Core S02-2 — Preview deploy ONLY (no push · no production).
 *
 * Adds ONE_KEY_CORE_ANALYSIS_COMPLETE=1 to Preview env (existing Preview env preserved).
 */
...
const OUT = join(FIX, "one-key-core-s02-2-preview-deploy-evidence.json");
...
async function main() {
  ...
  const evidence = {
    schema_version: "one-key-core-s02-2-preview-deploy-evidence-v1",
    slice: "ONE-KEY-CORE-S02-2-analysis_complete",
    ...
    next_step: "node scripts/one-key-core-s02-2-upload-3beat-preview-probe.mjs <preview-url>",
  };
  ...
  console.log(`Next: node scripts/one-key-core-s02-2-upload-3beat-preview-probe.mjs ${deploy.preview_url}`);
}
