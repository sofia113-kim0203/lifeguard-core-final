/**
 * ONE KEY Core S02-4 — Preview deploy ONLY (no push · no production).
 *
 * Adds ONE_KEY_CORE_RETURN_JUDGMENT=1 to Preview env (existing Preview env preserved).
 */
...
const OUT = join(FIX, "one-key-core-s02-4-preview-deploy-evidence.json");
...
async function main() {
  ...
  const evidence = {
    schema_version: "one-key-core-s02-4-preview-deploy-evidence-v1",
    slice: "ONE-KEY-CORE-S02-4-return_judgment",
    ...
    next_step: "node scripts/one-key-core-s02-4-upload-4beat-preview-probe.mjs <preview-url>",
  };
  ...
  console.log(`Next: node scripts/one-key-core-s02-4-upload-4beat-preview-probe.mjs ${deploy.preview_url}`);
}
