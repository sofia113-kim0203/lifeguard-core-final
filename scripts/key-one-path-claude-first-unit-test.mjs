...

{
  ...
  for (const c of voiceCases) {
    ...
    assert.equal(sys.includes("[KEY_CUSTOMER_SEAT]"), true, c.id);
    assert.equal(sys.includes("테스트 데이터"), true, c.id);
    assert.equal(sys.includes("없는 계약으로 말하지 않는다"), true, c.id);
    assert.equal(sys.includes("되묻기 전에"), true, c.id);
    assert.equal(sys.includes("대화 기록 연결"), true, c.id);
    assert.equal(sys.includes("기억에 없는 이유"), true, c.id);
    assert.equal(sys.includes("선택지를 다시 떠넘기지 않는다"), true, c.id);
    assert.equal(sys.includes("질문 하나를 한다"), true, c.id);
    assert.equal(sys.includes("고객이 맡기면"), true, c.id);
