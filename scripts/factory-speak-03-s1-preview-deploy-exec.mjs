/**
 * FACTORY-SPEAK-03-S1 — Preview deploy ONLY (no push · no production · no env change).
 *
 * Usage:
 *   node scripts/factory-speak-03-s1-preview-deploy-exec.mjs
 */
...
const OUT = join(FIX, "factory-speak-03-s1-preview-deploy-evidence.json");
...
async function main() {
  ...
  const evidence = {
    schema_version: "factory-speak-03-s1-preview-deploy-evidence-v1",
    slice: "FACTORY-SPEAK-03-S1-underwriting",
    ...
    next_step: "node scripts/factory-speak-03-s1-preview-customer-seat-audit.mjs <preview-url>",
  };
  ...
  console.log(`Next: node scripts/factory-speak-03-s1-preview-customer-seat-audit.mjs ${deploy.preview_url}`);
}
