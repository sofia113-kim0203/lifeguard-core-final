/**
 * E-2-1 follow-up — apply migration 032 only on existing staging (no 001–031 replay).
 *
 * Target: .e2-1-inventory.json staging_ref (must differ from production ref)
 * Migration: 032_profile_insurance_policies_extended_columns.sql
 *
 * Flags:
 *   --dry-run   Print planned actions only; no SQL execution
 *
 * Does NOT: delete/recreate staging, rerun 001–031, backfill, view 029 changes
...
const REPORT_PATH = path.join(ROOT, ".e2-1-staging-apply-032-report.json");
const MIGRATION_FILE = "032_profile_insurance_policies_extended_columns.sql";
...
async function verifyColumns(token, ref) {
  const colList = EXPECTED_COLUMNS.map((c) => `'${c}'`).join(", ");
  const sql = `
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_schema = 'public'
  AND table_name = 'profile_insurance_policies'
  AND column_name = ANY (ARRAY[${colList}])
ORDER BY column_name;`;
  return runSql(token, ref, sql, { label: "verify_columns" });
}
...
async function main() {
  ...
  console.log(`E2-1-APPLY-032_MODE=${DRY_RUN ? "dry_run" : "live"}`);
  console.log(`E2-1-APPLY-032_STAGING_REF=${stagingRef}`);
  console.log(`E2-1-APPLY-032_MIGRATION=${MIGRATION_FILE}`);
  console.log(`E2-1-APPLY-032_LAST_SUCCESS=${inventory?.last_success_migration ?? "null"}`);

  if (inventory?.last_success_migration !== "031_analysis_jobs_autopilot.sql") {
    console.log("E2-1-APPLY-032_WARN last_success_migration is not 031 — manual review recommended");
  }

  if (DRY_RUN) {
    console.log(`E2-1-APPLY-032_WOULD_APPLY migration=${MIGRATION_FILE} ref=${stagingRef}`);
    console.log(`E2-1-APPLY-032_WOULD_VERIFY columns=${EXPECTED_COLUMNS.join(",")}`);
    console.log("E2-1-APPLY-032_SKIP rerun 001-031=true");
    report.status = "dry_run_ok";
    report.finished_at = new Date().toISOString();
    fs.writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`, "utf8");
    console.log("E2-1-APPLY-032_STATUS=dry_run_ok");
    return;
  }
  ...
  const columns = await verifyColumns(token, stagingRef);
  report.column_verify = columns;
  ...
  console.log("E2-1-APPLY-032_STATUS=ok");
}

main().catch((error) => {
  console.error("E2-1-APPLY-032_FATAL", error instanceof Error ? error.message : String(error));
  process.exit(1);
});
