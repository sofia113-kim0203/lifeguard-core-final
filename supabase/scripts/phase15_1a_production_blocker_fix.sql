-- =============================================================================
-- LIFEGUARD Core — Phase 15-1A Production blocker fix (ONE-TIME PATCH)
-- Run after Phase 11–14 foundation scripts are applied. Does NOT modify 001–012.
-- Patches only: chunk approval sync, Claude model default. No new tables.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Patch: lifeguard_review_policy_chunk — sync real_policy_chunk_items status
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.lifeguard_review_policy_chunk(
  p_chunk_registry_id UUID,
  p_chunk_status      TEXT
)
RETURNS JSONB
LANGUAGE plpgsql
...
AS $lifeguard_review_policy_chunk$
DECLARE
  v_user_id          UUID;
  v_missing          JSONB := '[]'::JSONB;
  v_status           TEXT;
  v_chunk_id         UUID;
  v_synced_item_count INTEGER := 0;
BEGIN
  IF NOT public.lifeguard_is_admin() THEN
    RAISE EXCEPTION 'forbidden';
  END IF;

  IF p_chunk_registry_id IS NULL THEN
    RAISE EXCEPTION 'chunk_registry_id_required';
  END IF;

  IF p_chunk_status IS NULL OR length(trim(p_chunk_status)) = 0 THEN
    RAISE EXCEPTION 'chunk_status_required';
  END IF;

  IF NOT (trim(p_chunk_status) = ANY (public.lifeguard_policy_chunk_statuses())) THEN
    RAISE EXCEPTION 'invalid_chunk_status';
  END IF;

  SELECT id, chunk_status
  INTO v_chunk_id, v_status
  FROM public.policy_chunk_registry
  WHERE id = p_chunk_registry_id;

  IF v_chunk_id IS NULL THEN
    v_missing := v_missing || jsonb_build_array('chunk_not_found');
    RAISE EXCEPTION 'chunk_not_found';
  END IF;

  v_user_id := auth.uid();

  UPDATE public.policy_chunk_registry
  SET chunk_status = trim(p_chunk_status),
      reviewed_at = NOW(),
      reviewed_by = v_user_id
  WHERE id = p_chunk_registry_id;

  UPDATE public.real_policy_chunk_items
  SET chunk_status = trim(p_chunk_status)
  WHERE chunk_registry_id = p_chunk_registry_id;

  GET DIAGNOSTICS v_synced_item_count = ROW_COUNT;

  RETURN jsonb_build_object(
    'chunk_registry_id', p_chunk_registry_id,
    'chunk_status', trim(p_chunk_status),
    'real_policy_chunk_items_synced', v_synced_item_count,
...
COMMENT ON FUNCTION public.lifeguard_review_policy_chunk IS
  'Admin: review policy chunk status in registry and sync real_policy_chunk_items.';

-- ---------------------------------------------------------------------------
-- Patch: lifeguard_prepare_customer_ai_conversation — valid Claude model default
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.lifeguard_prepare_customer_ai_conversation(
  p_customer_id     UUID,
...
SECURITY DEFINER
SET search_path = public
AS $lifeguard_prepare_customer_ai_conversation$
DECLARE
  v_run_id                     UUID;
  v_user_id                    UUID;
  v_missing                    JSONB := '[]'::JSONB;
...
BEGIN
  ...
  BEGIN
    v_grounded_result := public.lifeguard_prepare_customer_grounded_conversation(
      p_customer_id,
      p_conversation_id,
      v_query_trim
    );
    ...
    v_claude_grounding_ready := COALESCE((v_grounded_result ->> 'claude_grounding_ready')::BOOLEAN, FALSE);
    v_claude_grounding_run_id := (v_grounded_result -> 'context_summary' ->> 'claude_grounding_run_id')::UUID;
  ...
  IF v_claude_grounding_run_id IS NULL AND v_execution_status <> 'failed' THEN
    BEGIN
  ...
  SET execution_status = v_execution_status,
      ...
      response_status = CASE
        ...
        ELSE 'pending'
      END,
      error_message = CASE WHEN v_execution_status = 'failed' THEN 'ai_conversation_preparation_failed' ELSE NULL END,
      completed_at = CASE WHEN v_execution_status IN ('ready', 'failed') THEN NOW() ELSE NULL END
  WHERE id = v_run_id;

  RETURN jsonb_build_object(
    'ai_conversation_run_id', v_run_id,
    'grounded_conversation_run_id', v_grounded_conversation_run_id,
    'claude_execution_run_id', v_claude_execution_run_id,
...
COMMENT ON FUNCTION public.lifeguard_prepare_customer_ai_conversation IS
  'Admin: prepare customer AI conversation via grounded context and Claude execution prep — no API call.';

-- ---------------------------------------------------------------------------
-- Patch: lifeguard_prepare_customer_real_policy_ai_conversation — valid model
-- ---------------------------------------------------------------------------
